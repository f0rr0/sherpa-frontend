...
class WikpediaInfoBox extends Component {
    ...
    getWikipediaData(query){
        ...
        console.log("http://api.geonames.org/wikipediaSearchJSON?maxRows=5&username=travelsherpa"+featureType+"&q="+query)
        ...
        //fetch(queryUrl, {
            ...
            for(var i=0;i<5;i++){
                var cleardQuery=this.removeDiacritics(query.toLowerCase());
                var cleardResponse=this.removeDiacritics(wikiResponse[i].title.toLowerCase());

                console.log(wikiResponse[i].title.toLowerCase(),'query::',query.toLowerCase());
                console.log(cleardQuery,'resp::',cleardResponse);
            ...
            console.log('location',
                Math.floor(this.props.coordinates.lat), wikiResult.lat,
                Math.floor(this.props.coordinates.lng), wikiResult.lng);
            ...
            if(
                ...
                if(this.props.type!="default"){
                    ...
                    fetch("http://lookup.dbpedia.org/api/search/KeywordSearch?QueryString="+cleardQuery, {
                        ...
                        finalDescription=finalDescription || wikiResult.summary.replace(" (...)","...");
                        console.log('final',finalDescription);
                        this.setState({"wikipediaDescription":finalDescription,"wikiURL":wikiResult.wikipediaUrl})
                    })
